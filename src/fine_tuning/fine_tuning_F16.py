...
def main():
    ...
    with mlflow.start_run() as run:
        ...
        model_path = Path(r"C:\Users\amari\OneDrive\Desktop\EdgeAI_Project\runs\rlhf\refined_rlhf_20251002_170303\weights\best.pt")
        ...
        try:
            # Load model with fallback for checkpoint structure
            try:
                model = YOLO(model_path)
            except KeyError as e:
                LOGGER.warning(f"Standard load failed ({e}). Loading state_dict directly...")
                state_dict = torch.load(model_path, map_location='cpu', weights_only=True)
                model = YOLO('yolov8n.pt')  # Load base YOLOv8n
                model.model.load_state_dict(state_dict, strict=False)  # Load custom weights
                LOGGER.info("Fallback load successful.")
            ...
            config = {
                ...
                'project': 'runs/rlhf',
                'name': f'refined_rlhf_fp16_{datetime.now().strftime("%Y%m%d_%H%M%S")}',
                'lr0': 0.0001,
                'lrf': 0.01,
                'patience': 50,
                'save': True,
                'verbose': True,
                'workers': 2,
                'amp': True,  # Enable mixed precision for FP16 compatibility
                'plots': True,
                'optimizer': 'AdamW',
                'cls': 0.5,
            ...
            LOGGER.info(f"\n📋 REFINED RLHF TRAINING CONFIGURATION (FP16 Compatible):")
            ...
            LOGGER.info(f"   16. ✅ FP16 mixed precision (amp=True) for quantization compatibility")
